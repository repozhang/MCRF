...
from models.bert_CRF import BertCrfForMultiLabel
...
def predict_multi(args, test_dataloader, model, tokenizer, prefix=""):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        tags2_r = show_results(tags_z[1], name='predict @ 2')
    ...
    write_to_tsv('./bert_crf_case_study.tsv', ['Number', 'Turn', 'predicts', 'labels'], cases)
    metrics = [F1Score(thresh=0.7, average='micro', task_type='multiclass'), 
                F1Score(thresh=0.7, average='macro', task_type='multiclass'), 
                JaccardScore(thresh=0.7, average='macro', task_type='multiclass')]
...
def main():
    ...
    label_list = [i for i in range(18)]
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            print("-"*20)
            model.to(args.device)
            predict(args, valid_data_li, model, tokenizer, prefix=prefix)
