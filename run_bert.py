...
from pybert.train.losses import BCEWithLogLoss, BCE_Loss
...
def run_train(args):
    ...
    valid_dataloader = DataLoader(valid_dataset, sampler=valid_sampler, batch_size=args.eval_batch_size,
                                  collate_fn=collate_fn)
    test_data = processor.get_test(config['data_dir'] / f"{args.data_name}.test.pkl")
    ...
    logger.info("label_list length") # add++
    # print(len(label_list)) # add++
    if args.resume_path:
        args.resume_path = Path(args.resume_path)
        model = BertForMultiLabel.from_pretrained(args.resume_path, num_labels=len(label_list))
    else:
        model = BertForMultiLabel.from_pretrained(config['bert_model_dir'], num_labels=len(label_list))
    print("----- Parameter Size -----")
    total_size=0
    for name,parameters in model.named_parameters():
        # print(name,':',parameters.size())
        # total_size += torch.prod(parameters.size())
        t = 1
    ...
    trainer = Trainer(args= args,model=model,logger=logger,criterion=BCE_Loss(),optimizer=optimizer,
                      scheduler=scheduler,early_stopping=None,training_monitor=train_monitor,
                      model_checkpoint=model_checkpoint,
                      batch_metrics=[AccuracyThresh(thresh=0.7)],
                      epoch_metrics=[nDCG(thresh=0.7, average='micro', task_type='multiclass'), 
                                     ExactRatio(thresh=0.7, average='micro', task_type='multiclass'),
                                     PatK(thresh=0.7, average='micro', task_type='multiclass'),
                                     F1Score(thresh=0.7, average='micro', task_type='multiclass'), 
                                     F1Score(thresh=0.7, average='macro', task_type='multiclass'),  
                                     JaccardScore(thresh=0.7, average='micro', task_type='multiclass'), 
                                     MultiLabelReport(id2label=id2label)])
    trainer.train(train_data=train_dataloader, valid_data=valid_dataloader, test_data=test_dataloader)

def run_test(args):
    from pybert.io.task_data import TaskData
    from pybert.test.predictor import Predictor, Single_Predictor
    data = TaskData()
    ...
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.train_batch_size,
                                 collate_fn=collate_fn)
    model = BertForMultiLabel.from_pretrained(config['checkpoint_dir'], num_labels=len(label_list))
    print("----- Parameter Size -----")
    total_size = 0
    for name,parameters in model.named_parameters():
        # print(name,':',parameters.size())
    ...
    #             F1Score(thresh=0.7, average='macro', task_type='multiclass'),  
    #             ]
    metrics = [nDCG(thresh=0.7, average='micro', task_type='multiclass'), 
             ExactRatio(thresh=0.7, average='micro', task_type='multiclass'),
             PatK(thresh=0.7, average='micro', task_type='multiclass'),
             F1Score(thresh=0.7, average='micro', task_type='multiclass'), 
             F1Score(thresh=0.7, average='macro', task_type='multiclass'),  
    ...
    # predictor = Single_Predictor(model=model,
    #                       logger=logger,
    #                       n_gpu=args.n_gpu, 
    #                       epoch_metrics=metrics)
    # print("-------print logits-------")
    # # logits, targets = predictor.predict(data=test_dataloader) # [seq_len, label_num]
    # results = predictor.predict(data=test_dataloader) # [seq_len, label_num]
    # print(results)
    # metrics = [Hamming(thresh=0.7, average='micro', task_type='multiclass'),
    #             nDCG(thresh=0.7, average='micro', task_type='multiclass'),
    #             ExactRatio(thresh=0.7, average='micro', task_type='multiclass'),
    #             PatK(thresh=0.7, average='micro', task_type='multiclass'),
    #             F1Score(thresh=0.7, average='micro', task_type='multiclass'), 
    #             F1Score(thresh=0.7, average='macro', task_type='multiclass'),  
    ...
    trainer = Trainer(args= args,model=model,logger=logger,criterion=BCE_Loss(),optimizer=None,
                      scheduler=None,early_stopping=None,training_monitor=None,
                      model_checkpoint=None,
                      batch_metrics=[AccuracyThresh(thresh=0.5)],
                      epoch_metrics=[nDCG(thresh=0.5, average='micro', task_type='multiclass'), 
                                     ExactRatio(thresh=0.5, average='micro', task_type='multiclass'),
                                     PatK(thresh=0.5, average='micro', task_type='multiclass'),
                                     F1Score(thresh=0.5, average='micro', task_type='multiclass'), 
                                     F1Score(thresh=0.5, average='macro', task_type='multiclass'),  
                                     JaccardScore(thresh=0.5, average='micro', task_type='multiclass'), 
                                     MultiLabelReport(id2label=id2label)])
    # trainer.valid_epoch(data=test_dataloader)
    # trainer.test_epoch(data=test_dataloader)
    trainer.test_multi_epoch(data=valid_dataloader)
    trainer.test_multi_epoch(data=test_dataloader)
    # end
...
def main():
    ...
    parser.add_argument('--data_name', default='MDMDdata', type=str)
    ...
    parser.add_argument("--learning_rate", default=2e-5, type=float) # single-label
    # parser.add_argument("--learning_rate", default=1e-2, type=float) # multi-label
    ...
    if args.do_data:
        ...
        idxs, targets, sentences = data.read_data(raw_data_path=config['raw_data_path'],
                                            preprocessor=EnglishPreProcessor(),
                                            is_train=True)
        data.train_val_split(Idx=idxs, X=sentences, y=targets, shuffle=True, stratify=False,
